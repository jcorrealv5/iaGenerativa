...
archivoPeloRubio = os.path.join(rutaPeloOpuesto, "B0.png")
...
gen.load_state_dict(torch.load("preentrenados/GAN/CelebA/gen_blond.pth"))
...
if(video.isOpened()):
    while(True):
        ...
        imgRubio = cv2.resize(imgRubio, img.size())
        imagenes = cv2.hconcat([img, imgRubio])
        if(rpta):
            cv2.imshow("Pelo Normal Vs Rubio", imagenes)
            key = cv2.waitKey(1)
            if(key==ord("s")):
                break
        else:
