...
print("Demo 60: Entrenamiento de un Modelo Cycle-GAN de CelebA para Sonrisas")
...
def test(epoca,i,A,B,fake_A,fake_B):
    carpeta = "C:/Data/Python/2026_01_IAG/Demos/preentrenados/GAN/Sonrisa/Epoca" + str(epoca)
...
dataset = LoadData(root_A=["datasets/Sonrisa/No/"],root_B=["datasets/Sonrisa/Si/"],transform=transforms)
...
for epoca in range(100):
    ...
    torch.save(gen_A.state_dict(), "preentrenados/GAN/Sonrisa/gen_sinrisa_" + str(epoca+1) + ".pth")
    torch.save(gen_B.state_dict(), "preentrenados/GAN/Sonrisa/gen_conrisa_" + str(epoca+1) + ".pth")
torch.save(gen_A.state_dict(), "preentrenados/GAN/Sonrisa/gen_sinrisa.pth")
torch.save(gen_B.state_dict(), "preentrenados/GAN/Sonrisa/gen_conrisa.pth")

horaFin = datetime.now()
tiempoSeg = (horaFin - horaInicio).total_seconds()
print(f"Se creo y entreno el Modelo Cycle GAN de Sonrisas en {tiempoSeg} seg")