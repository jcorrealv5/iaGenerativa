...
if(video.isOpened()):
    while(True):
        ...
        if(rpta):
            ...
            imgRubio = imgPeloRubio.copy()
            augmentations = transforms(image=imgRubio, image0=img)
            ...
            imgRubio = cv2.resize(imgRubio, (imgOriginal.shape[1],imgOriginal.shape[0]))
            imgRubio = cv2.cvtColor(imgRubio, cv2.COLOR_RGB2BGR)
            imagenes = cv2.hconcat([imgOriginal, imgRubio])
